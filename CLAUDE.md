...
- Чекбокс **«📋 Чек листы»** в топбаре (`#chk-mode`, `CHECK_MODE`). При включении:
  - справа появляется **панель чек-листов** (`#chk-panel`): список чек-листов, попадающих
    ...
    повторный клик снимает подсветку (`SEL_CHK`). У внутренних чек-листов — кнопка **🗑 удалить**
    (только админ, с подтверждением) → POST `deleteChecklist` (мягкое удаление, см. эндпоинты).
