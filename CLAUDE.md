...
- Чекбокс **«📋 Чек листы»** в топбаре (`#chk-mode`, `CHECK_MODE`). При включении:
  - справа появляется **панель чек-листов** (`#chk-panel`): список чек-листов, попадающих
    ...
    на всю группу; остальным — бейдж), кнопка **📷 N** — открывает **модалку фотографий**
    (`#ph-overlay`: сетка миниатюр через `drive.google.com/thumbnail?id=`, клик — открыть файл;
    внизу кнопка «＋ Добавить» → выбор файлов `#cp-photo-inp`, multiple, только админ; файлы
    шлются последовательно по одному POST `addChecklistFile`), и кнопка **🗑 удалить**
